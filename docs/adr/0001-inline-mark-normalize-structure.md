...
这带来一个核心问题：如果每个 inline mark 都自己处理输入、识别 source、同步 mark 和隐藏 delimiter，很快会出现多套规则。它们会在嵌套、冲突、inline code isolation、删除 delimiter、Markdown round trip 等场景里互相打架。
...
```mermaid
flowchart TD
  A["创建一个 inline feature 文件"] --> B["声明 mark schema、Markdown parse 规则、inline source 扫描规则"]
  ...
  G --> I["Markdown serializer 原样输出 retained inline source text"]
```
...
- Markdown serialization 对 inline text 只读取文档中保留的 source text，不触发编辑态转换，不根据 marks 合成或 escape inline Markdown。

## 创建一个 feature

新增 inline mark 时，应该创建独立的 feature 文件。这个文件描述该 feature 的完整能力，但不拥有全局编辑流程。

一个 inline mark feature 通常提供三类信息：

- 它在 ProseMirror 中对应什么 semantic mark。
- Markdown parser 如何把外部 Markdown 读成保留 source delimiter text 的 semantic mark。
- 编辑时如何从当前 textblock 的 source text 中识别这个 mark 的 source。

例如 highlight feature 关心的是 `==text==` 如何成立、成立后内容对应 highlight mark，以及 parser 如何把外部 Markdown 读成保留 delimiter 的 PM text。它不应该自己决定是否要覆盖 italic、是否要隐藏 delimiter、是否要清理其它 mark。这些跨 feature 的判断属于统一 normalize 流程。
...
Inline normalize 只消费 registry 里的 inline source 扫描规则。这样新增 feature 后，它自然进入同一套 parse/normalize/decorate 管线，而不是额外挂一条旁路。
...
Markdown parse 负责把外部 Markdown 读成 ProseMirror 文档。对 inline mark，它必须同时保留 source delimiter text，并给内容附加 semantic marks；进入编辑器后的 live source 识别仍由 `parseInline` 和 normalize 负责。

Markdown serialize 负责把当前 PM doc 写回 Markdown。Block node 仍按结构语义输出，例如 heading 的 `# `、blockquote 的 `> `、list marker、code fence，以及 hard break 的 `\\\n`。

Inline serialization 是纯 source-retained：

- text node 直接输出 `node.text ?? ""`。
- `hard_break` 继续输出 Markdown hard break。
- serializer 不读取 inline marks，不排序 marks，不维护 active mark stack。
- serializer 不根据 semantic mark 状态合成 delimiter。
- serializer 不因为 semantic mark 缺失而 escape `*`、`[`、`` ` ``、`~`、`#` 等普通 inline Markdown 字符。

这保证 `**1**`、`hello *world`、`[x](y)`、`` `a*b` `` 这些 inline source 怎么存在于文档文本里，就怎么序列化出去。是否识别成 mark 是 parser / normalize 的责任，不由 serializer 掩饰或修正。
...
- Serializer 对 inline text 保持 source-retained，不合成 delimiter，不 escape 普通 inline Markdown 字符。

## 后果

这个设计让新增 inline mark 的路径变得稳定：实现 feature，注册到 registry，交给统一 parse/normalize 流程处理。

它也把复杂性集中在正确的位置。单个 feature 不需要知道所有其它 feature 的行为；跨 feature 的冲突、嵌套和隔离由统一入口处理。显示层不需要理解 Markdown 语法；它只消费 normalize 的结果。Markdown parser 负责把外部 Markdown 读成 source-retained 文档；serializer 不参与 inline 语义修复，只输出文档事实。
